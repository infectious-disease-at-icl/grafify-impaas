...
output$graphsHelpOpen <- renderText({
  HTML(paste0(tags$div(
    tags$ul(
      ...
      tags$li(
        tags$strong("Box 7"),
        "appears with options to reorder groups when the X-axis variable is categorical."
      ),
      tags$li(
        tags$strong("Box 7"),
...
output$pickvariables <- renderText({
  HTML(paste0(
    ...
    tags$div(
      ...
      tags$ul(
        ...
        tags$li(
          "If you change variables for graphs or anlayses, remember to click relevant action buttons. Graphs and analyses do not automatically update when variables are changed."
        ),
        tags$li(
...
#  output$graphs_head <- renderText({
#    if(input$startBtn > 0) {
#      HTML(paste0("<h4>","Your graphs","</h3>"))
#    }
#  })
#  output$qq_head <- renderText({
#    if(input$startBtn > 0) {
#      HTML(paste0("<h4>","QQ plot of model residuals","</h3>"))
#    }
#  })
output$qq_head <- renderText({
  if (input$startBtn > 0) {
...
output$emmeans_head <- renderText({
  if (input$startBtn > 0) {
    HTML(
      paste0(
        "<h4>",
...
output$contrasts_head <- renderText({
  if (input$startBtn > 0) {
    HTML(
      paste0(
        "<h4>",
...
output$Instr_Data <- renderText({
  HTML(paste0(tags$div(
    ...
    tags$ul(
      ...
      layout_columns(col_widths = 10, card("", tagList(
        #h4("Types of graphs with grafify"),
        tags$img(
          src = "shiny5_longwide_tabs.png",
          align = "left",
...
output$Instr_Graphs <- renderText({
  HTML(paste0(
    tags$div(tags$h5("Types of Graphs"), mainPanel2.1),
    ...
    tags$div(
      ...
      tags$ul(
        ...
        tags$li(
          tags$strong(
            "Note: all variables chosen in Boxes 1-3, and log-transformation used from Box 8, will be used in data anlayses. If any levels within categorical X-axis are dropped from graphs in Box 7, they will not be used in analyses."
          )
        ),
        tags$li(
    ...
    tags$div(tags$h6("Types of Graphs"), mainPanel2.2),
